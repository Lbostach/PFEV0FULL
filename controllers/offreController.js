    ...
    try {
        ...
        transporter.sendMail(mailOptions, (error, info) => {
            if (error) {
                ...
                res.status(500).json({ message: 'Une erreur s\'est produite lors de l\'envoi de l\'email au recruteur' });
            } else {
                console.log('Email sent:', info.response);
                res.status(200).json({ message: 'Candidature envoyée avec succès au recruteur' });
            }
        });
    } catch (error) {
    ...
    try {
        ...
        transporter.sendMail(mailOptions, (error, info) => {
            if (error) {
                ...
                console.log('Email sent:', info.response);
                res.status(200).json({ message: 'Candidature envoyée avec succès au recruteur', candidature: nouvelleCandidature });
            }
        });
    } catch (error) {
        console.error('Error applying for offer:', error);
